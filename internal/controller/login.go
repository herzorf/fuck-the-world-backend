...
func Login(c *gin.Context) {
	...
	if result.Error != nil {
		...
		if !user.CheckPassword(loginInfo.Password) {
			utils.RespondJSON(c, http.StatusBadRequest, "密码错误", nil)
			return
		}
		if !*user.IsActive {
			utils.RespondJSON(c, http.StatusForbidden, "该用户不可用，请联系管理员", nil)
			return
		}
		jwtString, err := FTWJwt.GenerateJWT(user)
		if err != nil {
			utils.HandleError("生成JWT失败", err)
		}
		utils.RespondJSON(c, http.StatusOK, "登录成功", gin.H{
			"jwt": jwtString,
		})
		return
	}
